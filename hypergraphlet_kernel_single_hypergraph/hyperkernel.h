...
#ifndef __HYPERKERNEL_H__
#define __HYPERKERNEL_H__
...
class HyperKernels  {
public:
    HyperKernels() : NORMALIZE(false), VERBOSE(false), SF(0.0), ELM(0), EM(0) {}
    ~HyperKernels()  {}
     
    /** Read a vertex- and hyperedge-labeled hypergraph and a list of vertex of interest. */
    void read_hypergraph(string, string, string, const vector<unsigned> &);

    /** Read a vertex- and hyperedge-labeled dual hypergraph and a list of vertex of interest. */
    void read_dual_hypergraph(string, string, string, const vector<unsigned> &);

    /** Read a probability similarity matrix for each vertex label so that we can weight each vertex label substitution. */
    void read_sim_vlm_matrix(string filename);

    /** Read a probability similarity matrix for each hyperedge label so that we can weight each hyperedge label substitution. */
    void read_sim_elm_matrix(string filename);

    /** Stores the class labels of examples, if provided. **/
    inline void set_labels(const vector<int> &l)  { labels = l; }

    /** Compute cumulative random walk hyperkernel matrix. */
    void compute_random_walk_cumulative_matrix(int steps, double restart);

    /** Compute random walk hyperkernel matrix. */
    void compute_random_walk_matrix(int steps, double restart);

    /** Compute label substitutions hyperkernel matrix. */
    void compute_label_mismatch_matrix();

    /** Compute hyperedge indels hyperkernel matrix. */
    void compute_edge_mismatch_matrix();

    /** Compute edit distance (with 1 operation) hyperkernel matrix. */
    void compute_edit_distance_matrix();

    /** Compute edit distance (with 2 operations) hyperkernel matrix. */
    ...
    inline void set_edge_label_mismatches_alphabet(string e_alphabet)  { EDGES_ALPHABET = e_alphabet; }
...
private:
	/** Returns the cumulative random walk hyperkernel between two rooted neighborhoods. */
    float random_walk_cumulative(SimpleHypergraph &hg, unsigned hg1_root, unsigned hg2_root, int steps, double restart);

	/** Returns the random walk hyperkernel between two rooted neighborhoods. */
    float random_walk(SimpleHypergraph &hg, unsigned hg1_root, unsigned hg2_root, int steps, double restart);
